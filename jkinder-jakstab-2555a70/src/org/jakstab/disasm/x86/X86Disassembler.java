...
public class X86Disassembler implements Disassembler, X86Opcodes {
		...
		try {
			...
			if (opcode == 0x0f) {
				...
				if ((prefixes & PREFIX_REPNZ) != 0) {
					 ...
					 * does not work with prefixed standard 2-Byte instructions! else { instrDecoder
					 * = twoByteTable[opcode]; }
					 */
				if (instrDecoder == null)
			...
			if (instr == null) {
				logger.error("Decoder " + instrDecoder.getClass().toString() + " for opcode " + Long.toHexString(opcode)
						+ " returned null instruction!");
				return null;
			}
	...
	private final int getPrefixes() {
		...
		while (isPrefix) {
			...
			case 0x67:
				...
				break;
			/*
			 * This had to be removed, since FWAIT is really an instruction and can appear
			 * on its own. Mnemonics like FSTSW are only macros, anyway, and a jump to the
			 * FNSTSW part after 0x9B is ok. To make this really work, we would need to
			 * check for an earlier FWAIT instruction while parsing instructions like
			 * FNSTSW. case 0x9b: prefixes |= PREFIX_FWAIT; break;
			 */
			default:
				isPrefix = false;
			}
	...
	private static final InstructionDecoder oneByteTable[] = {
			...
			new InstructionDecoder("pushl", ADDR_REG, ES), new InstructionDecoder("popl", ADDR_REG, ES),
			/* 08 */
			...
			new InstructionDecoder("pushl", ADDR_REG, CS), null, /* 0x0f extended opcode escape */
			/* 10 */
			...
			new InstructionDecoder("pushl", ADDR_REG, SS), new InstructionDecoder("popl", ADDR_REG, SS),
			/* 18 */
			...
			new InstructionDecoder("pushl", ADDR_REG, DS), new InstructionDecoder("popl", ADDR_REG, DS),
			/* 20 */
			...
			new ArithmeticDecoder("andS", ADDR_REG, EAX, ADDR_I, v_mode, Operation.AND), null, /* SEG es prefix */
			...
			new ArithmeticDecoder("subS", ADDR_REG, EAX, ADDR_I, v_mode, Operation.SUB), null, /* SEG CS prefix */
			...
			new ArithmeticDecoder("xorS", ADDR_REG, EAX, ADDR_I, v_mode, Operation.XOR), null, /* SEG SS prefix */
			...
			new InstructionDecoder("cmpS", ADDR_REG, EAX, ADDR_I, v_mode), null, /* SEG DS prefix */
			...
			/* 50 */
			new InstructionDecoder("pushS", ADDR_REG, EAX), new InstructionDecoder("pushS", ADDR_REG, ECX),
			new InstructionDecoder("pushS", ADDR_REG, EDX), new InstructionDecoder("pushS", ADDR_REG, EBX),
			new InstructionDecoder("pushS", ADDR_REG, ESP), new InstructionDecoder("pushS", ADDR_REG, EBP),
			new InstructionDecoder("pushS", ADDR_REG, ESI), new InstructionDecoder("pushS", ADDR_REG, EDI),
			/* 58 */
			new InstructionDecoder("popS", ADDR_REG, EAX), new InstructionDecoder("popS", ADDR_REG, ECX),
			new InstructionDecoder("popS", ADDR_REG, EDX), new InstructionDecoder("popS", ADDR_REG, EBX),
			new InstructionDecoder("popS", ADDR_REG, ESP), new InstructionDecoder("popS", ADDR_REG, EBP),
			new InstructionDecoder("popS", ADDR_REG, ESI), new InstructionDecoder("popS", ADDR_REG, EDI),
			/* 60 */
			new InstructionDecoder("pusha"), new InstructionDecoder("popa"),
			new InstructionDecoder("boundS", ADDR_G, v_mode, ADDR_E, v_mode),
			new InstructionDecoder("arpl", ADDR_E, w_mode, ADDR_G, w_mode), null, /* seg fs */
			null, /* seg gs */
			null, /* op size prefix */
			...
			new InstructionDecoder("pushl", ADDR_I, b_mode), /*
																 * push of byte really pushes 4 bytes (or 2 with data
																 * prefix)
																 */
			...
			/* 70 */
			new ConditionalJmpDecoder("jo", ADDR_J, b_mode), new ConditionalJmpDecoder("jno", ADDR_J, b_mode),
			new ConditionalJmpDecoder("jb", ADDR_J, b_mode), new ConditionalJmpDecoder("jae", ADDR_J, b_mode),
			new ConditionalJmpDecoder("je", ADDR_J, b_mode), new ConditionalJmpDecoder("jne", ADDR_J, b_mode),
			new ConditionalJmpDecoder("jbe", ADDR_J, b_mode), new ConditionalJmpDecoder("ja", ADDR_J, b_mode),
			/* 78 */
			new ConditionalJmpDecoder("js", ADDR_J, b_mode), new ConditionalJmpDecoder("jns", ADDR_J, b_mode),
			new ConditionalJmpDecoder("jp", ADDR_J, b_mode), new ConditionalJmpDecoder("jnp", ADDR_J, b_mode),
			new ConditionalJmpDecoder("jl", ADDR_J, b_mode), new ConditionalJmpDecoder("jnl", ADDR_J, b_mode),
			new ConditionalJmpDecoder("jle", ADDR_J, b_mode), new ConditionalJmpDecoder("jg", ADDR_J, b_mode),
			/* 80 */
			new GRPDecoder(null, 0), new GRPDecoder(null, 1), new GRPDecoder(null, 0), // undocumented
			new GRPDecoder(null, 2), new InstructionDecoder("testb", ADDR_E, b_mode, ADDR_G, b_mode),
			...
			new InstructionDecoder("leaS", ADDR_G, v_mode, ADDR_E, v_mode), // JK:
																			...
																			// introduced
																			// errors!
			new MoveDecoder("movw", ADDR_SEG, w_mode, ADDR_E, w_mode), new InstructionDecoder("popS", ADDR_E, v_mode),
			/* 90 */
			new InstructionDecoder("nop"), new MoveDecoder("xchgS", ADDR_REG, ECX, ADDR_REG, EAX),
			...
			/* 98 */
			new InstructionDecoder("cwtl"), new InstructionDecoder("cltd"), new CallDecoder("lcall", ADDR_DIR, p_mode),
			// Made fwait an instruction, too, to handle multiple fwait
			// prefixes!
			new InstructionDecoder("fwait"),
			// null, /* fwait */
			new InstructionDecoder("pushfS"), new InstructionDecoder("popfS"), new InstructionDecoder("sahf"),
			...
			/* c0 */
			new GRPDecoder(null, 3), new GRPDecoder(null, 4), new RetDecoder("ret", ADDR_I, w_mode),
			new RetDecoder("ret"), new InstructionDecoder("lesS", ADDR_G, v_mode, ADDR_E, p_mode), // JK:
																									// changed
																									// 0
																									// to
																									// p_mode
	...
	private static final InstructionDecoder twoByteTable[] = {
			/* 00 */
			new GRPDecoder(null, 13), new GRPDecoder(null, 14),
			new InstructionDecoder("larS", ADDR_G, v_mode, ADDR_E, w_mode),
			new InstructionDecoder("lslS", ADDR_G, v_mode, ADDR_E, w_mode), null, null, new InstructionDecoder("clts"),
			null,
			/* 08 */
			new InstructionDecoder("invd"), new InstructionDecoder("wbinvd"), null, null, null, null, null, null,
			/* 10 */ // SSE
	...
	private static final InstructionDecoder twoBytePrefixF2Table[] = {
			...
			null, null, null, null, null, null, null, null,
			/* e0 */
			null, null, null, null, null, null, new SSEInstructionDecoder("cvtpd2dq", ADDR_V, dq_mode, ADDR_W, pd_mode),
			null,
			/* e8 */
			null, null, null, null, null, null, null, null,
			/* f0 */
	...
	private static final InstructionDecoder twoBytePrefixF3Table[] = {
			...
			null, null, null, null, null, null, null, null,
			/* e0 */
			null, null, null, null, null, null, new SSEInstructionDecoder("cvtdq2pd", ADDR_V, pd_mode, ADDR_W, dq_mode),
			null,
			/* e8 */
			null, null, null, null, null, null, null, null,
			/* f0 */
